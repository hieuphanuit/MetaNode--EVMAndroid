...
Java_com_example_nativec_MainActivity_deploy(
   ...
   // format argument to right data type
    ...
    jmethodID constructor = jenv->GetMethodID(clazz, "<init>", "([B[B[B)V");
...
Java_com_example_nativec_MainActivity_call(
    ...
    const auto output =
            run_and_check_result(env, caller_address, contract_address, input);
    ...
    jbyteArray jb_output = jenv->NewByteArray(output.size());
    jenv->SetByteArrayRegion( jb_output, 0, output.size(), (const jbyte*)output.data() );

    jclass clazz = jenv->FindClass("com/example/nativec/EVMCallRs");
    jmethodID constructor = jenv->GetMethodID(clazz, "<init>", "([B[B)V");
    // Create an instance of clazz
    jobject obj = jenv->NewObject(clazz, constructor, jb_output, jb_storage);
