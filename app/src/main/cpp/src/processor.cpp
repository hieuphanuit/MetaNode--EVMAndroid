    ...
    private:
        ...
        void extcodesize()
        {
          ...
          // TODO: call to android or ios to get code relate to smart contract or code do fetch
            ...
            jclass jniGetDataClass = ctxt->jenv->FindClass("com/example/nativec/EVMGetData");
            jmethodID getCodeMethod = ctxt->jenv->GetStaticMethodID(jniGetDataClass, "GetCode",
                                                                    "([B)[B");
            jbyteArray jb_address = ctxt->jenv->NewByteArray(20u);
            ctxt->jenv->SetByteArrayRegion (jb_address, 0, 20u, (jbyte*)b_address);

            jbyteArray getCodeResult = (jbyteArray)ctxt
                    ->jenv
                    ->CallStaticObjectMethod(jniGetDataClass, getCodeMethod, jb_address);

            char* b_deployed_code = (char*)ctxt
                    ->jenv
                    ->GetByteArrayElements(getCodeResult, NULL);

            uint deployed_code_length = ctxt
                    ->jenv
                    ->GetArrayLength(getCodeResult);

            std::vector<uint8_t> deployed_code((uint8_t*)b_deployed_code, (uint8_t*)b_deployed_code + deployed_code_length);
            // get balance
            jmethodID getBalanceMethod = ctxt->jenv->GetStaticMethodID(jniGetDataClass, "GetBalance",
                                                                    "([B)[B");
            jbyteArray getBalanceResult = (jbyteArray)ctxt
                    ->jenv
                    ->CallStaticObjectMethod(jniGetDataClass, getBalanceMethod, jb_address);

            char* b_balance = (char*)ctxt
                    ->jenv
                    ->GetByteArrayElements(getBalanceResult, NULL);

            uint256_t balance = from_big_endian((uint8_t *)b_balance, 32u);

            // get storage
            jmethodID getStorageMethod = ctxt->jenv->GetStaticMethodID(jniGetDataClass, "GetStorage",
                                                                       "([B)Ljava/lang/String;");
            jobject getStorageResult = (jcharArray)ctxt
                    ->jenv
                    ->CallStaticObjectMethod(jniGetDataClass, getStorageMethod, jb_address);
            const char* charStorage = ctxt->jenv->GetStringUTFChars((jstring) getStorageResult, NULL);
